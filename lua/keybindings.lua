...
if keys.s_windows ~= nil and keys.s_windows.enable then
  ...
  keymap("n", skey.height_decrease, ":resize -2<CR>")
  keymap("n", skey.height_increase, ":resize +2<CR>")
...
pluginKeys.gitsigns_on_attach = function(bufnr)
  ...
  local function map_(mode, l, r, opts)
    opts = opts or {}
    opts.buffer = bufnr
  ...
  -- toggle
  map_("n", "<leader>gtd", gs.toggle_deleted)
  map_("n", "<leader>gtb", gs.toggle_current_line_blame)
  -- Text object
  map_({ "o", "x" }, "ig", ":<C-U>Gitsigns select_hunk<CR>")
end
